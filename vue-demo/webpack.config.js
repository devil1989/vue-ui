...
var filePathS=isDev?"build":'dist';
...
var hashStr=+new Date()+""+Math.floor(Math.random()*100000000000000);//url设置hash，用于去图片缓存

//url-loader参数，css中的image的url路径=output.publicPath+imagePath+文件名-hash.图片文件后缀
var urlLoaderImagePath=("url-loader?name="+imagePath+"[name].[ext]?hash="+hashStr+"&limit=2048")+(isDev?"":("&publicPath=")+publicPath);

var fontPath=filePath+"/assets/fonts/";//html里面的图片路径
// use: 'url-loader?prefix=font/&limit=10000'
var urlLoaderFontPath = ("url-loader?name="+fontPath+"[name].[ext]?hash="+hashStr+"&prefix=font/&limit=10000");//+(isDev?"":("&publicPath=")+publicPath);

//图片loader中的publicPath参数：发布的时候，图片的基础路径（html的图片和css中的图片都是用这个路径，所以最好写完整的绝对路径，因为css和html不在同一个目录下） [图片和其他可以拆分]

function getPublishPath(){//这个是html中引用的静态资源的url前缀
    if(isQa){
        return "//qares.hjfile.cn/crm/ocean/";
    }
    else if(isYz){
        return "//yzres.hjfile.cn/crm/ocean/";
    }else if(isDev){
        return "//localhost:8080/";
    }
    else if(isPro){
        return "//res.hjfile.cn/crm/ocean/";
    }
}

// function getCssAndJsPublishPath(){//res.hjfile.cn/crm/ocean/
//     if(isQa){
//         return "//qares.hjfile.cn/crm/ocean/";
//     }
//     else if(isYz){
...
function getAllHtmlWebpackPlugin (){//多个页面入口，需要有新建多个HtmlWebpackPlugin插件对象
    ...
    keys.forEach(function(val){
        ...
        var configObj={//生成html （热插拔：配置1（没有这个动态生成html文件，热插拔无法正常监控））
            ...
            chunks:[isDev?"common_dev":"common",val],
            hash:true
        };
        if(val!=="common"&&val!=="common_dev"){//common不需要指定template，因为只想用common包的js和css，html用不到
            configObj.template=__dirname+"/src/assets/pages/"+val+".html";//对应的html，必须制定，否则就产生一个内容为空，只有js引用的html文件
        }
...
var commonConfig = {
    ...
    output: {//输出文件
        ...
        //publicPath:"/crm/ocean/",
        publicPath:publicPath,//publicPath指定的是构建后在html里src和href的路径的基础地址（HtmlWebpackPlugin这个插件就是用这个publicPath来生成对应的html）
        ...
        loaders: [
            {
                ...
                loader:urlLoaderImagePath//小于2kb的直接转为base64,指定对应的图片路径=publicPath+imagePath
            },
            ...
            // },
            {
                ...
                loader: urlLoaderFontPath//这个是iconfont的地址，
//                use: 'url-loader?prefix=font/&limit=10000'

                // resourceQuery: /([a-z0-9]+)?/,
//                use: [{
//                    loader: "file-loader",
//                    options: {
//                        name: "[path][name].[ext]"
...
//                }]
    ...
    resolve: {
        modules: [path.resolve(__dirname, '../common/component'), path.resolve(__dirname, '../common/libs'),
        path.resolve(__dirname, "../common/utils"), path.resolve(__dirname, "../common/lion"), path.resolve(__dirname, '../node_modules')],
        alias: {
            'vue': __dirname+'/../node_modules/vue/dist/vue.js',//用node_modules文件夹里面的vue/dist/vue.js来编译，vue执行的时候只需要运行时，打包的时候做编译即可，可以节省vue的size
            'vue-min': __dirname+'/../node_modules/vue/dist/vue.runtime.min.js'//vue.runtime.min.js只包含运行时
